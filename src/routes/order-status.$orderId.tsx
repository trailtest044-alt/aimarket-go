...
export const Route = createFileRoute("/order-status/$orderId")({ component: OrderStatusPage });

function OrderStatusPage() {
  const { orderId } = Route.useParams();
  const { data: order, isLoading } = useQuery({ queryKey: ["order", orderId], queryFn: () => getOrderById(orderId), refetchInterval: 5000 });
  return (
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-3xl px-4 py-12 sm:px-6">
        ...
        <div className="glass mt-6 animate-rise-lg rounded-3xl p-8">
          {isLoading ? (
            ...
            <div className="py-10 text-center">
              ...
              <p className="mt-2 text-muted-foreground">We couldn't find order <span className="font-mono">{orderId}</span>.</p>
              <Link to="/products" className="btn-primary mt-6 inline-block rounded-xl px-6 py-3 text-sm font-semibold">Browse Products</Link>
            </div>
          ) : (
...
function OrderDetailsWithDelivery({ order }: { order: Order }) {
  const { data: delivery } = useQuery({
    ...
    enabled: order.status === "approved" || order.status === "delivered",
    ...
    <>
      <div className="flex flex-col items-center text-center">
        <div className={`grid h-20 w-20 place-items-center rounded-full ${meta.iconBg}`}>
          {order.status === "pending" ? <div className="waiting-ring scale-75" /> : <meta.Icon className={`h-10 w-10 ${meta.iconColor}`} />}
        </div>
        <h1 className="mt-5 font-display text-3xl font-bold">{meta.title}</h1>
        <p className="mt-2 max-w-md text-sm text-muted-foreground">{meta.message}</p>
        <div className={`mt-5 inline-flex items-center gap-2 rounded-full border px-4 py-1.5 text-xs font-bold ${meta.badge}`}>
          <meta.Icon className="h-3.5 w-3.5" /> {meta.label}
        </div>
      </div>

      <div className="mt-6">
        <SupportHelpSection title="Need help with this order?" />
      </div>

      <div className="mt-8 rounded-2xl border border-border bg-white/4 p-5 text-sm">
        <h2 className="text-xs font-bold uppercase tracking-[0.18em] text-muted-foreground">Order details</h2>
...
function DeliveryCard({ delivery }: { delivery?: DeliveryPayload | null }) {
  ...
  return (
    <div className="mt-6 animate-rise rounded-3xl border border-success/30 bg-success/6 p-5">
      ...
      {delivery ? (
        <div className="mt-5 space-y-4">
          ...
          {delivery.imageUrl && (
            <div className="rounded-2xl border border-border bg-white/5 p-4">
              <div className="mb-3 text-xs font-bold uppercase text-muted-foreground">Image guide</div>
...
function MediaVideo({ url }: { url: string }) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-white/5 p-4">
      <div className="mb-3 text-xs font-bold uppercase text-muted-foreground">Video guide</div>
...
function statusMeta(status: Order["status"]) {
  switch (status) {
    case "pending":
      return { Icon: Clock, title: "Order Pending", label: "Pending", message: "Waiting for admin approval. We'll update this page automatically.", iconBg: "bg-warning/12", iconColor: "text-warning", badge: "border-warning/30 bg-warning/10 text-warning" };
    case "approved":
      return { Icon: CheckCircle2, title: "Order Approved", label: "Approved", message: "Your delivery is ready below.", iconBg: "bg-primary/12", iconColor: "text-primary", badge: "border-primary/30 bg-primary/10 text-primary" };
    case "delivered":
