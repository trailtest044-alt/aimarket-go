    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-5xl px-4 py-12 sm:px-6">
        ...
        <div className="mt-8 space-y-5">
          {results?.length === 0 && <div className="glass rounded-3xl p-8 text-center text-muted-foreground">No order found with this Transaction ID or Order ID. Please check the code and try again.</div>}
  ...
  return (
    <div className="glass rounded-3xl p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><div className="text-xs uppercase tracking-wider text-muted-foreground">Order</div><h2 className="mt-1 text-xl font-bold">{order.productName}</h2><div className="mt-1 font-mono text-xs text-muted-foreground">{order.id}</div></div>
        <div className={`inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs font-semibold ${meta.badge}`}><meta.Icon className="h-3.5 w-3.5" /> {meta.label}</div>
      </div>
      ...
      {order.status === "pending" && <div className="mt-5 rounded-2xl border border-warning/25 bg-warning/10 p-4 text-sm text-warning">Waiting for admin approval. Track again using the same Transaction ID or Order ID.</div>}
      {(order.status === "approved" || order.status === "delivered") && <DeliveryCard delivery={delivery} />}
      {order.status === "rejected" && <div className="mt-5 rounded-2xl bg-destructive/10 p-4 text-sm text-destructive">Order rejected. Please contact support.</div>}
    </div>
  );
}
...
function DeliveryCard({ delivery }: { delivery?: DeliveryPayload | null }) {
  ...
  return (
    <div className="mt-6 rounded-3xl border border-success/30 bg-success/5 p-5">
      <div className="flex items-center gap-2 text-sm font-bold text-success"><ShieldCheck className="h-4 w-4" /> Delivery Details & Instructions</div>
      <p className="mt-2 text-sm text-muted-foreground">This is the same delivery area you get after admin approval. Keep the login details private.</p>
      {delivery ? <div className="mt-5 space-y-4">
        <Info label="Login Email" value={delivery.email || ""} />
        <div className="rounded-2xl bg-white/75 p-4">
          <div className="text-xs font-semibold uppercase text-muted-foreground">Password</div>
          <div className="mt-2 flex items-center justify-between gap-3"><span className="break-all font-mono text-lg font-bold text-slate-900">{show ? password : "•".repeat(Math.max(8, password.length || 8))}</span><div className="flex gap-2"><button type="button" onClick={() => setShow(!show)} className="rounded-lg bg-secondary px-2 py-1 text-xs">{show ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button>{password && <CopyButton value={password} />}</div></div>
        </div>
        {hasMedia && <div className="rounded-3xl border border-primary/15 bg-white/65 p-4"><div className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-muted-foreground"><FileText className="h-4 w-4 text-primary" /> Product Instruction Area</div>{instruction && <div className="rounded-2xl bg-white/80 p-4"><div className="text-xs font-semibold uppercase text-muted-foreground">Instructions</div><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-800">{instruction}</p></div>}{delivery.videoUrl && <MediaVideo url={delivery.videoUrl} />}{delivery.imageUrl && <MediaImage url={delivery.imageUrl} />}</div>}
      </div> : <div className="mt-4 rounded-xl border border-dashed border-border bg-white/60 p-4 text-sm text-muted-foreground">Delivery is approved but details are loading. Try again in a moment or contact support.</div>}
    </div>
  );
}
...
function MediaVideo({ url }: { url: string }) {
  ...
  return <div className="mt-4 rounded-2xl bg-white/80 p-4"><div className="mb-3 flex items-center gap-2 text-xs font-semibold uppercase text-muted-foreground"><PlayCircle className="h-4 w-4 text-primary" /> Video Guide</div>{yt ? <iframe src={src} className="aspect-video w-full rounded-xl" allowFullScreen title="Delivery video" /> : <video src={src} controls className="max-h-[520px] w-full rounded-xl" />}</div>;
}
function MediaImage({ url }: { url: string }) { return <div className="mt-4 rounded-2xl bg-white/80 p-4"><div className="mb-3 flex items-center gap-2 text-xs font-semibold uppercase text-muted-foreground"><ImageIcon className="h-4 w-4 text-primary" /> Image Guide</div><img src={url} alt="Delivery guide" className="max-h-[520px] w-full rounded-xl object-contain" /></div>; }
function Info({ label, value }: { label: string; value: string }) { return <div className="rounded-2xl bg-white/75 p-4"><div className="text-xs font-semibold uppercase text-muted-foreground">{label}</div><div className="mt-2 flex items-center justify-between gap-3"><span className="break-all font-mono text-lg font-bold text-slate-900">{value || "Not provided"}</span>{value && <CopyButton value={value} />}</div></div>; }
