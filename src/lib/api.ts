...
export type PromoCode = {
  ...
  percentValue?: number;
  fixedAmount?: { bd: number; pk: number; world: number };
  minOrderAmount?: { bd: number; pk: number; world: number };
  productIds?: string[];
  allowedRegions?: PriceRegion[];
  allowedPaymentMethods?: Array<"bangladesh" | "pakistan" | "binance">;
...
export async function getPromoCodes(): Promise<PromoCode[]> {
  const data = await http<{ promoCodes: PromoCode[] }>("/admin/promo-codes");
  return data.promoCodes || [];
}

export async function createPromoCode(input: Partial<PromoCode>): Promise<PromoCode> {
  const data = await http<{ promoCode: PromoCode }>("/admin/promo-codes", {
    method: "POST",
...
export async function updatePromoCode(id: string, input: Partial<PromoCode>): Promise<PromoCode> {
  const data = await http<{ promoCode: PromoCode }>(`/admin/promo-codes/${id}`, {
    method: "PATCH",
    body: JSON.stringify(input),
  });
  return data.promoCode;
