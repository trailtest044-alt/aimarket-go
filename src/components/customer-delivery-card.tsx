...
export function CustomerDeliveryCard({ orderId, delivery, productName, productLogoUrl, productIcon, fetchLoginCode = fetchLatestLoginCode }: { orderId: string; delivery?: DeliveryPayload | null; productName?: string; productLogoUrl?: string; productIcon?: string; fetchLoginCode?: (orderId: string) => Promise<LoginCodeResult> }) {
  ...
  const getCodeAvailable = mode === "login_code" && delivery?.canFetchLoginCode !== false;
  ...
  return (
    <div className="mt-6 rounded-3xl border border-success/30 bg-success/6 p-5">
      ...
      {productName && <div className="mt-4 flex items-center gap-3 rounded-2xl border border-success/20 bg-card/70 p-3">
        <ProductLogo logoUrl={productLogoUrl} icon={productIcon || "✨"} name={productName} className="h-11 w-11 rounded-xl bg-white/5" emojiClassName="text-xl" />
        <div><div className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Delivered AI / product</div><div className="mt-0.5 font-bold text-foreground">{productName}</div></div>
      </div>}
          ...
          {getCodeAvailable && (
            <div className="rounded-2xl border border-border bg-card p-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-muted-foreground">
                    <MailCheck className="h-4 w-4 text-success" /> Login with Get Code
                  </div>
                  <p className="mt-1 text-xs text-muted-foreground">Use the email above to sign in, then click Get code after the code arrives in that delivered inbox.</p>
                  {delivery.getCodeAccessExpiresAt && <p className="mt-1 text-[11px] font-semibold text-success">Available until {new Date(delivery.getCodeAccessExpiresAt).toLocaleString()}</p>}
                </div>
