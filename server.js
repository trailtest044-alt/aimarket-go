...
async function seedAdmin() {
  ...
  if (!email || password.length < 10) {
    console.log("⚠️  No admin exists. Set ADMIN_EMAIL and ADMIN_PASSWORD (10+ chars) env vars and restart to create the first owner.");
    return;
  }
  await Admin.create({
    email,
...
mongoose.connect(process.env.MONGODB_URI)
  .then(async () => {
    await seedAdmin();
    app.listen(PORT, () => console.log(`🚀 AIMarket Pro running on port ${PORT} (storefront + /admin + /api)`));
  })
